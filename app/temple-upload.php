...
if(isset($_REQUEST['upload'])){
	...
	if(strtolower($ext) == "csv")
	{
        ...
        while (($getData = fgetcsv($file)) !== FALSE) {
	        if($i>0 && $getData[10] != ''){
           		$DatabaseCo->dbLink->query("INSERT INTO `temples`(`banner`, `my_stery`, `god_id`, `title`, `temple_place`, `log_date`, `sthalam`, `puranam`, `photos`, `varnam`, `highlights`, `sevas`, `open_time`, `close_time`, `gallery_image`, `country`, `state`, `city`, `address`, `order_by`, `speciality`, `time`, `speciality_title`,`status`) VALUES ('$getData[0]','$getData[1]','$getData[2]','$getData[3]','$getData[4]','$log_date','$getData[5]','$getData[6]','$getData[7]','$getData[8]','$getData[9]','$getData[10]','$getData[11]','$getData[12]','$getData[13]','$getData[14]','$getData[15]','$getData[16]','$getData[17]','$getData[18]','$getData[19]','$getData[20]','$getData[21]','unapproved')");$j++;
	       	}$i++;
	   	}
	}
	header("location:temple-upload.php?count=$j");
}
?>
<style>
    .icon-container {
